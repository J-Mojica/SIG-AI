...
def load_zeros_and_ones():
    ...
    new_train_x_vector = np.zeros((new_train_x.shape[0], new_train_x.shape[1]*new_train_x.shape[2], 1))
    new_test_x_vector = np.zeros((new_train_x.shape[0], new_train_x.shape[1]*new_train_x.shape[2], 1))
    for i in range(new_train_x.shape[0]):
        new_train_x_vector[i] = matrix_to_vector(new_train_x[i])
    for i in range(new_test_x.shape[0]):
        new_test_x_vector[i] = matrix_to_vector(new_test_x[i])

    return (new_train_x_vector, new_train_y), (new_test_x_vector, new_test_y)
